...
def main() -> int:
    ...
    if args.command == "validate-ablation":
        ...
        print(json.dumps(report, indent=2))
        return 0
    if args.command == "run-temporal":
        from yakseopdong.temporal import run_temporal

        print(json.dumps(run_temporal(repository_root()), indent=2))
        return 0
    if args.command == "validate-temporal":
    ...
    if args.command == "run-robustness":
        from yakseopdong.robustness import run_robustness

        print(json.dumps(run_robustness(repository_root()), indent=2))
        return 0
    if args.command == "validate-robustness":
